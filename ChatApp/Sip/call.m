...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
    ...
    if (ci.state == PJSIP_INV_STATE_DISCONNECTED) // Session is terminated.
    {
        ...
        sip_ring_stop([[AppDelegate theDelegate] pjsipConfig]);
    ...
    else if (ci.state == PJSIP_INV_STATE_EARLY)
    {
            ...
            ci.media_status == PJSUA_CALL_MEDIA_NONE)
        {
            ...
            sip_ringback_start([[AppDelegate theDelegate] pjsipConfig]);
        }
	}
                             ...
                             pjsip_rx_data *rdata)
{
    ...
    /* FIXME: Start ringback */
    sip_ring_start([[AppDelegate theDelegate] pjsipConfig]);
...
typedef struct struct_codecs {
    __unsafe_unretained NSString *param;
    pj_str_t pjsip_name;
} Codecs;
...
pj_status_t sip_startup(app_config_t *app_config)
{
    ...
    pj_ansi_snprintf(tmp, 80, "Adore SIP Client v%s/%s", pj_get_version(), PJ_OS_NAME);
    ...
    app_config->log_cfg.console_level = (int)val;
    app_config->log_cfg.level = (int)val;
    if (val != 0)
    {
#if defined(CYDIA) && (CYDIA == 1)
        NSArray *filePaths = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
        NSString *path = [NSString stringWithFormat:@"%@/Adore SIP Client", [filePaths objectAtIndex:0]];
#else
    ...
    if (val < 0 || val > 65535)
    {
        ...
        [[AppDelegate theDelegate] displayParameterError:
    ...
    app_config->udp_cfg.port = (int)val;
    
    pjsua_transport_config_default(&(app_config->rtp_cfg));
    app_config->rtp_cfg.port = (int)[[NSUserDefaults standardUserDefaults]
...
pj_status_t sip_connect(pj_pool_t *pool, pjsua_acc_id *acc_id)
{
    ...
    uname  =[[[NSUserDefaults standardUserDefaults] stringForKey:@"username"] UTF8String];
    
    authname  =[[[NSUserDefaults standardUserDefaults] stringForKey:@"authname"] UTF8String];
    contactname  =[[[NSUserDefaults standardUserDefaults] stringForKey:@"contact"] UTF8String];
    passwd = [[[NSUserDefaults standardUserDefaults] stringForKey:@"password"] UTF8String];
    //server = "sip.yelloworchidphone.com";//[[[NSUserDefaults standardUserDefaults] stringForKey:@"server"] UTF8String];
    server = [[[NSUserDefaults standardUserDefaults] stringForKey:@"server"] UTF8String];
    */
    
    uname = [[[NSUserDefaults standardUserDefaults]objectForKey:@"Username"] UTF8String];
    passwd =[[[NSUserDefaults standardUserDefaults]objectForKey:@"Password"] UTF8String];
    //_Realm = [[NSUserDefaults standardUserDefaults]objectForKey:@"SIP_REALM"];
    authname = uname;
    contactname = uname;
    server =[[[NSUserDefaults standardUserDefaults]objectForKey:@"SIP_SERVER"] UTF8String];
    //_SIPPort=[[NSUserDefaults standardUserDefaults]objectForKey:@"SIP_PORT"];
    ...
    if ((status = pjsua_verify_sip_url(acc_cfg.id.ptr)) != 0)
    {
        PJ_LOG(1,(THIS_FILE, "Error: invalid SIP URL '%s' in local id argument",
                  acc_cfg.id));
        [[AppDelegate theDelegate] displayParameterError: @"Invalid value for username or server."];
        return status;
    }
    ...
    if ((status = pjsua_verify_sip_url(acc_cfg.reg_uri.ptr)) != 0)
    {
        PJ_LOG(1,(THIS_FILE,  "Error: invalid SIP URL '%s' in registrar argument",
                  acc_cfg.reg_uri));
        [[AppDelegate theDelegate] displayParameterError: @"Invalid value for server parameter."];
        return status;
    }
    ...
    acc_cfg.reg_timeout =1800;//[[NSUserDefaults standardUserDefaults] integerForKey:@"regTimeout"];
    ...
    // Keep alive interval
    acc_cfg.ka_interval = (int)[[NSUserDefaults standardUserDefaults] integerForKey:
                           @"kaInterval"];
    ...
    while (anObject = [enumerator nextObject])
    {
        ...
        if (proxy && strlen(proxy))
        {
            ...
            if ((status = pjsua_verify_sip_url(pj_proxy.ptr)) != 0)
            {
                ...
                [[AppDelegate theDelegate] displayParameterError: @"Invalid value for proxy parameter."];
                continue;
            }
    ...
    //NSLog(@"%@",[NSNumber numberWithInt:acc_id]);
    ...
    *acc_id = aid;
    ...
    if (status != PJ_SUCCESS)
    {
        ...
        [[AppDelegate theDelegate] displayParameterError: @"Error adding new account."];
    }
#endif
    return status;
}
                     ...
                     pjsua_call_id *call_id)
{
    ...
    // sip_domain ="sip.yelloworchidphone.com";[[[NSUserDefaults standardUserDefaults] stringForKey:
    //            @"server"] UTF8String];
...
pj_status_t sip_add_account(NSDictionary *account, pjsua_acc_id *acc_id)
{
    ...
    // server = "sip.yelloworchidphone.com";[[account objectForKey: @"server"] UTF8String];
    ...
    // FIXME: gestion du message 423 dans pjsip
    acc_cfg.reg_timeout =1800;//[[account objectForKey: @"regTimeout"] intValue];
    // FIXME : verify in settings view
    if (acc_cfg.reg_timeout < 1 || acc_cfg.reg_timeout > 3600)
    {
        PJ_LOG(1,(THIS_FILE,
